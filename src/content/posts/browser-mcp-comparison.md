---
title: "브라우저를 AI에게 맡기는 두 가지 방식 — Chrome DevTools MCP vs Playwright MCP"
...
Chrome DevTools MCP는 실행 중인 Chrome과 연결 상태에 의존한다. 내가 보고 있던 페이지를 그대로 넘겨받는 편리함은 환경 의존성과 한 몸이다. Chrome 136부터 원격 디버깅 포트에 기본 사용자 데이터 디렉터리를 그대로 쓰는 방식이 제한됐기 때문에, 별도 `--browser-url` 연결과 평소 프로필 재사용을 같은 것으로 생각하면 안 됐다. 기존 세션이 목적이라면 당시 문서의 auto-connect 방식을 따로 확인해야 했다.
