...
만약 3.0이상의 버전을 다운 받는 다면 webrick 을 설치해주도록 하자.[^1] <br/>
[^1] : webrick은 Ruby의 라이브러리이고, Ruby 3.0 ver부터 gem에 webrick가 포함되지 않기 때문에 serve를 동작 시 오류가 생겨날 수 있다. 오류가 생겨나고 webrick install을 해주어도 상관은 없습니다.
```shell
gem install webrick
```
...
아래와 같은 명령어로 bundle[^2]을 설치하고 업데이트하면 된다. <br/>

[^2]: bundle은 영어명 그대로 무언가를 묶어주는 역할을 한다. 서로 참조관계가 있는 파일을 묶어주는 것 즉, 의존성이 있는 모듈을 묶어주기 때문에 시간도 절약되고 파일 크기도 줄어든다.
...
```
local 호스팅은 http://127.0.0.1:4000 또는 http://localhost:4000에서 확인할 수 있다.
