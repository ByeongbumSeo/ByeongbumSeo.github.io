---
title: "중복 요청을 유니크 제약으로 한 번만 처리하기"
...
동시에 두 INSERT가 와도 유니크 인덱스는 하나만 성공시킨다. Spring JDBC에서는 나머지 요청이 `DuplicateKeyException`을 받는다.
...
**동시 INSERT에서 발생한 `DuplicateKeyException`은 DB가 두 번째 요청을 막았다는 신호로 처리할 수 있다.**

다만 `DuplicateKeyException`이라는 타입만 보고 모든 중복을 같은 성공으로 처리하면 안 된다. 테이블에 유니크 제약이 여러 개라면 다른 키가 충돌했을 수도 있다. INSERT가 충돌시킬 수 있는 제약을 좁히거나, 제약 이름과 원인 예외를 확인해 `uk_reward_claim` 위반만 "이미 처리됨"으로 바꾼다.
...
애플리케이션의 사전 확인과 DB 유니크 키는 같은 중복 기준을 사용해야 한다.
...
유니크 키를 추가하기 전에 무엇을 하나의 요청으로 볼지부터 맞춰야 한다.
...
이 경우에는 중복 요청에 오류 응답을 돌려준다. HTTP 상태와 오류 코드는 API 정책에 맞게 정하면 된다.

### 같은 요청에 기존 결과를 반환하는 경우
...
INSERT에 성공한 요청만 후속 지급을 한다. 중복이면 아무것도 더 하지 않고 기존 처리 결과를 반환한다. 클라이언트 관점에서는 같은 요청을 여러 번 보내도 최종 상태가 같아진다.[^upsert]
...
외부 시스템까지 걸친 처리라면 로컬 트랜잭션만으로는 부족하다. DB 변경과 발행할 메시지를 함께 기록하는 방식, 상태 머신과 재처리 정책도 필요하다. 유니크 키는 중복 INSERT를 막지만 외부 작업까지 함께 성공시키지는 않는다.
...
동일한 `user_id`에 `campaign_id = NULL`인 행이 여러 개 들어갈 수 있다. 유니크 키를 믿기 전에 구성 컬럼의 `NULL` 허용 여부를 확인해야 한다.
...
- SELECT 사전 확인은 빠른 실패와 친절한 응답을 위해 사용한다.
...
**한 번만 처리해야 하는 입력에서는 예상한 유니크 제약의 `DuplicateKeyException`만 중복 요청으로 바꿔야 한다.**

[^upsert]: upsert나 insert-ignore 계열 문법을 쓸 수도 있지만, 어떤 제약 위반까지 무시하는지 분명해야 한다. 모든 무결성 오류를 한꺼번에 삼키면 원인을 놓친다.
