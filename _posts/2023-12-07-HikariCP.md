---
title: HikariCP, Connection Pool, JDBC(작성중)
...
### 데이터 베이스 커넥션 풀(Database Connection Pool, DBCP) 사용 이유
...
6. Closing (1)
 
즉, **서버가 DB에 연결하기 위한 Connecting 비용이 가장 큰 비율을 차지**한다. 이처럼 Connection을 생성하는 작업은 비용이 많이 드는 작업이다. 이를 보완할 수 있는 방법이 바로 **Connection Pool**이다.
...
maximum-pool-size: 풀의 최대 크기를 나타냅니다.
connection-timeout: 데이터베이스 연결을 시도하는 시간 초과 시간을 지정합니다.
connection-init-sql: 연결이 초기화될 때 실행할 SQL 쿼리를 설정합니다.
validation-timeout: 유효성 검사를 수행하는 시간 초과 시간을 지정합니다.
minimum-idle: 최소한으로 유지할 연결의 수를 지정합니다.
idle-timeout: 유휴 연결이 제거되기 전 대기할 수 있는 최대 시간을 설정합니다.
max-lifetime: 연결이 풀에서 유지될 수 있는 최대 시간(ms)을 나타냅니다.
