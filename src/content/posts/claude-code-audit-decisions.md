---
title: "Claude Code 감사 로그에서 화이트리스트 대신 블랙리스트를 선택한 이유"
slug: "claude-code-audit-decisions"
description: "변경 명령 화이트리스트보다 읽기 명령 블랙리스트를 택한 이유와, 복합 Bash 명령을 안전하게 판별해야 하는 경계를 설명한다."
kind: "tech"
...
로그 자체의 위험도 남는다. 사용자 프롬프트 일부와 Bash 명령, 절대경로를 평문으로 저장하므로 토큰이나 자격증명이 명령 인자에 들어가면 함께 기록될 수 있다. 현재 플러그인은 파일 권한도 기본 `umask`에 맡긴다.[^log-permission]
...
[^log-permission]: 기록 범위를 넓힐수록 보관 기간과 마스킹을 정하고, 로그 디렉터리와 파일에는 `0700`, `0600`처럼 소유자만 접근할 수 있는 권한을 검토해야 한다.
