---
title: "클라이언트 재시도 중복 차감을 조건부 UPDATE로 막기"
...
`OPEN`인 행을 다시 `OPEN`으로 바꾸는 UPDATE까지 성공으로 취급한 것이 직접적인 원인이었다.
...
**`state = 'CLOSED'` 확인과 `OPEN` 변경을 한 UPDATE에서 처리하면, 두 번째 요청은 연결의 affected-rows 설정과 관계없이 `0`을 받는다.**
