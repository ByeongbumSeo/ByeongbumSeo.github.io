---
title: "AI에게 '테스트 짜줘'라고 했더니 런던파였다 — 고전파 테스트 철학을 에이전트 스킬에 심기까지"
slug: "ai-agent-testing"
description: "내부 협력 객체를 과도하게 mock하던 테스트를 점검하고 고전파 테스트 원칙을 AI 에이전트 스킬에 반영한 과정을 정리한다."
kind: "tech"
...
글을 읽고 끝내지 않고, 실제로 사용 중인 테스트와 에이전트 스킬을 열어봤다.
...
내가 정리한 핵심은 협력 객체를 어디까지 실제로 실행하느냐였다.
...
고전파가 모든 테스트를 API E2E로 만들라는 뜻은 아니다. 순수 계산은 Spring 없이 단위 테스트할 수 있다. DB와 매퍼를 포함해야 의미가 있는 서비스 동작은 통합 테스트로 실행한다. 중요한 것은 테스트 레벨보다 **빠르고 통제 가능한 내부 코드를 이유 없이 가짜로 바꾸지 않는 것**이었다.
...
## 한 번 설명하는 대신 다음 요청의 기본값을 바꿨다

이번에 바꾼 것은 테스트 코드 몇 개보다 에이전트의 기본 선택이었다.

1. 기존 테스트를 보고 mock 경계를 확인했다.
2. 실제로 실행할 내부 코드와 대역으로 둘 외부 경계를 나눴다.
3. 이 기준을 테스트 스킬과 예시에 반영했다.

"DAO를 mock하지 마"라고 리뷰에서 반복하는 것보다, 다음 테스트를 만들 때 먼저 읽는 가이드 한 번을 고치는 편이 효과적이었다. 아티클의 주장에 동의하는 것과 작업 방식의 기본값을 바꾸는 것은 다른 일이다. 내게 가장 남은 교훈은 후자였다.
