---
title: "LIKE 검색을 MySQL FULLTEXT ngram으로 바꾸기 전에"
slug: "mysql-fulltext-ngram"
description: "한글 부분 검색을 개선할 때 LIKE, B-tree, FULLTEXT, ngram이 각각 해결하는 문제와 도입 전에 확인할 운영 조건을 정리한다."
kind: "tech"
...
전체 기간을 검색해야 한다면 MySQL의 `FULLTEXT` 인덱스와 ngram 파서를 쓸 수 있지 않을까 싶어 동작을 정리했다. 결론부터 말하면 실제 기능에는 적용하지 않았다. 기존 검색 범위와 사용 빈도를 생각하면 제한된 `LIKE`를 유지하는 편이 단순했고, `FULLTEXT`는 인덱스 하나를 더하는 변경이 아니라 검색의 의미까지 바꾸는 선택이었기 때문이다.
...
MySQL의 range optimization 문서도 와일드카드로 시작하지 않는 상수 패턴만 B-tree의 range 조건으로 다룬다고 설명한다. 따라서 `LIKE '%mysql%'`는 `title` 인덱스에서 검색 범위를 만들지 못한다. 다만 이것이 언제나 테이블 전체를 처음부터 끝까지 읽는다는 뜻은 아니다. 다음처럼 다른 조건으로 후보를 먼저 줄이면, MySQL은 그 후보에 대해서만 `LIKE`를 검사할 수 있다.
...
이 방식은 전체 본문 검색을 빠르게 만든 것이 아니다. PK 범위로 읽을 행을 제한해 비용의 상한을 낮춘 것이다. 최근 데이터만 확인하는 운영 화면이라면 이 제약이 요구사항과 잘 맞을 수 있다. 반대로 오래된 글까지 빠짐없이 찾아야 한다면 결과 자체가 달라지므로 성능 우회책으로만 볼 수 없다.
...
Boolean mode가 `LIKE '%검색어%'`와 완전히 같다는 뜻은 아니다. 공백 처리, 불용어, collation, 검색어 길이에 따라 경계 조건이 달라진다. ngram 와일드카드 검색에서 접두사가 토큰 크기보다 짧으면 그 접두사로 시작하는 ngram을 가진 행을 찾는다. 접두사가 토큰보다 길면 검색어를 ngram 구문으로 바꾸고 `*`는 무시한다. 일반적인 접두사 검색과 같은 의미로 볼 수 없으므로 실제 데이터로 결과 집합을 비교해야 한다.
...
InnoDB는 full-text 색인을 위해 보조 테이블과 `FTS_DOC_ID`를 사용한다. 이미 데이터가 든 테이블에 처음 인덱스를 추가하면서 해당 컬럼이 없다면 테이블을 다시 만드는 작업이 발생할 수 있다. 또한 full-text 대상 컬럼의 변경은 트랜잭션 커밋 시 처리되고 검색에는 커밋된 데이터만 보인다. 읽기 쿼리 하나의 속도만으로는 알 수 없는 운영 특성이다.
...
검토한 화면은 검색 범위가 제한된 내부 운영 기능이었다. 전체 이력을 검색하는 정확성보다 최근 데이터를 단순하게 찾는 용도가 더 컸고, 기존 PK 범위 조건으로 검사할 행 수도 제한할 수 있었다. 이 상황에서 `FULLTEXT`를 추가하면 검색 품질 테스트와 인덱스 운영 비용이 새로 생긴다. 얻을 이점이 그 비용보다 크다는 근거는 아직 없었다.
...
그때의 비교 대상은 `LIKE`와 `FULLTEXT` 두 개만이 아니다. 제한된 범위의 `LIKE`, 기본 `FULLTEXT`, ngram `FULLTEXT`를 각각 같은 검색 시나리오로 검증해야 한다. 형태소 분석, 동의어, 오타 보정 같은 요구가 중요하다면 MySQL 안에서 해결할 문제인지부터 다시 판단해야 한다.

`LIKE '%...%'`가 느리다는 사실만으로 ngram을 선택할 수는 없다. B-tree가 만들 수 있는 범위, FULLTEXT가 바꾸는 검색 의미, ngram이 만드는 문자 토큰, 그리고 인덱스를 유지하는 비용을 나눠 봐야 한다. 이번 검토에서 가장 크게 남은 결론은 “어떤 인덱스가 빠른가”보다 “우리가 원하는 검색이 정확히 무엇인가”를 먼저 정해야 한다는 점이었다.
