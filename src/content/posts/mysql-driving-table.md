---
title: "드라이빙 테이블은 작은 테이블이면 끝일까"
slug: "mysql-driving-table"
description: "MySQL 조인에서 먼저 읽는 입력이 왜 중요한지 살펴보고, 테이블 크기보다 필터링 결과·인덱스·통계·조인 알고리즘을 함께 봐야 하는 이유를 정리한다."
kind: "tech"
...
여기서 먼저 읽어 반복을 시작하는 `outer_input`을 흔히 드라이빙 테이블, 나중에 조회하는 `inner_input`을 드리븐 테이블이라고 부른다. 이때 outer와 inner는 nested loop의 바깥쪽·안쪽이라는 뜻이다. SQL의 `LEFT OUTER JOIN`에서 말하는 outer와는 구분해야 한다.
...
그래서 “작은 쪽을 먼저”보다 정확한 기준은 다음과 같다.

> 바깥쪽 입력에서 적은 행을 만들고, 각 행으로 안쪽 입력을 싸게 찾을 수 있는가?

안쪽 입력의 조인 컬럼에 적절한 인덱스가 중요한 이유도 여기에 있다. 반복할 때마다 전체 테이블을 읽는 것과 인덱스로 필요한 범위만 찾는 것은 비용이 전혀 다르다.

## 현재 MySQL에서 FROM 순서는 실행 순서가 아니다

과거 규칙 기반 옵티마이저를 설명할 때는 인덱스 유무와 FROM 절 위치에 따른 우선순위를 외우기도 했다. “조인 인덱스가 없는 테이블을 먼저 읽어, 인덱스가 있는 테이블을 안쪽에서 조회한다”는 규칙도 nested loop의 비용을 이해하는 휴리스틱으로는 의미가 있다.

하지만 이를 현재 MySQL의 고정 규칙으로 사용하면 안 된다. MySQL은 통계와 비용 모델을 이용해 접근 방식과 조인 순서를 고른다. 같은 SQL도 데이터 분포, 인덱스, 통계와 서버 버전에 따라 다른 계획이 나올 수 있다. 일반적인 inner join에서는 SQL에 테이블을 적은 순서만 보고 드라이빙 테이블을 판단할 수 없다.
...
위 출력은 읽는 법을 보여주기 위한 형태다. 안쪽 `activity_log` 조회의 `loops=1000`은 바깥 입력의 1천 행마다 한 번씩 실행됐다는 뜻이다. 반복 iterator의 `actual rows`는 loop당 평균이므로, 전체 작업량은 대략 `rows × loops`로 읽는다. 위 예시라면 안쪽 조회가 반환한 행은 약 3천 건이다. 예상 행 수와 실제 행 수가 크게 다르면 “왜 이 테이블을 먼저 골랐지?”보다 통계와 선택도 추정이 맞는지부터 확인하는 편이 낫다.
...
`EXPLAIN FORMAT=TREE`에서는 `Inner hash join`, `Hash`, `Table scan` 구조를 통해 `Hash` 아래의 build 입력과 다른 쪽의 probe 입력을 구분할 수 있다. 여기에 `EXPLAIN ANALYZE`를 사용하면 실제 시간과 행 수도 더해진다. 실제 메모리 사용량이나 디스크 spill 여부가 이 출력에 직접 표시되는 것은 아니다. 다만 해시가 `join_buffer_size` 안에 들어가지 않으면 MySQL이 디스크를 사용할 수 있으므로 별도의 운영 지표와 함께 확인해야 한다. 모든 조인을 드라이빙·드리븐 두 단어로만 설명하면 이런 실행 방식이 빠진다.
...
드라이빙 테이블은 여전히 조인 비용을 이해하는 좋은 출발점이다. 다만 결론은 “항상 작은 테이블부터”가 아니다. **필터링 뒤의 바깥 입력이 작고, 안쪽 접근이 싸며, 그 판단을 실제 실행 계획의 반복 횟수로 확인했는가**까지 봐야 한다.
