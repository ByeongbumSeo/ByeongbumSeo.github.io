---
title: "IntelliJ가 no usage라고 한 getter를 지웠더니 API가 깨졌다"
slug: "jackson-reflection-usage"
description: "정적 분석에 잡히지 않는 Jackson 직렬화 호출을 추적하고 응답 모델의 getter를 안전하게 정리하는 기준을 정리한다."
kind: "tech"
...
애플리케이션 코드에 `response.getThumbnailUrl()`이라는 호출문이 없어도 런타임 직렬화기는 이 메서드를 사용한다. 일반적인 Jackson Databind 경로에서는 리플렉션을 통한 접근도 일어난다. 최적화 모듈을 사용하면 실제 호출 수단은 달라질 수 있지만, 정적 호출 그래프에서 보이지 않는다는 본질은 같다.
...
따라서 `no usage`는 “실행되지 않는다”가 아니라 “IDE가 정적으로 찾은 사용처가 없다”는 뜻으로 읽어야 했다.
...
이번 문제는 IntelliJ의 오진이 아니었다. 정적 분석이 다루는 세계와 프레임워크가 런타임에 만드는 호출 사이의 경계였다. 회색 코드는 삭제 후보일 수 있지만, 프레임워크 규약에 걸린 메서드라면 먼저 실행 경로부터 확인해야 한다.
