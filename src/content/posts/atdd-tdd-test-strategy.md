...
*인수 조건을 Given–When–Then 형식으로 정리한 예시*
...
단위 테스트로 검증할 조건은 해당 테스트에서 TDD[^tdd-cycle] 주기를 바로 반복한다. 통합 테스트로 검증할 조건은 통합 테스트를 Red로 둔 채 필요한 모듈을 같은 주기로 구현하고, 마지막에 통합 테스트를 Green으로 만든다.[^outside-in]
...
[^atdd-bdd]: BDD와 ATDD 모두 구현 전에 기대하는 동작과 조건을 구체화한다. BDD는 주어진 상황에서 시스템이 보여야 할 행동을 공통 언어로 설명하는 데 초점을 두고, ATDD는 이해관계자가 합의한 인수 조건을 개발과 검증의 기준으로 삼는다. 이 글에서는 AI에게 구현을 맡기기 전에 완료 조건을 확정하고, 구현 과정에서 이를 임의로 바꾸지 않는 것이 목적이므로 ATDD의 관점을 차용했다.

[^tdd-cycle]: TDD는 특정 테스트 종류가 아니라 실패하는 테스트를 먼저 작성하고(Red), 테스트를 통과할 만큼 구현한 뒤(Green), 코드를 정리하는(Refactor) 개발 주기다.

[^outside-in]: 이 진행 순서는 outside-in TDD와 일부 닮아 있다. outside-in TDD는 일반적으로 mock과 상호작용 검증을 활용해 협력 객체의 인터페이스를 먼저 설계하는 런던파 TDD에서 많이 사용되는 접근이다. 이 글에서는 그중 바깥의 완료 조건에서 세부 구현으로 내려가는 순서만 가져왔다.
