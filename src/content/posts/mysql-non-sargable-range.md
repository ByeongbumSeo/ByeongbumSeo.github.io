---
title: "인덱스가 있는데 왜 풀스캔이 날까"
slug: "mysql-non-sargable-range"
description: "행마다 달라지는 날짜 경계가 인덱스를 무력화한 원인을 찾고, 상위집합 하한으로 원래 결과를 보존하며 range scan을 유도한 과정을 정리한다."
kind: "tech"
...
이런 조건을 흔히 non-sargable이라고 부른다. 인덱스가 존재하는지와 그 인덱스를 검색 인자로 사용할 수 있는지는 다른 문제다.

일반 B-Tree 인덱스에서 비슷한 형태도 자주 나온다. 표현식에 맞춘 generated column이나 functional index를 따로 둔 경우는 예외다.
...
## 원래 조건을 유지한 채 더 느슨한 조건을 추가했다

원래 날짜 조건은 비즈니스 규칙이라 상수로 바꿀 수 없었다. 대신 그 조건을 모두 포함하는 고정 하한을 하나 더 붙였다.
...
추가 조건은 정확한 판정을 맡지 않는다. 모든 행의 `extra_days` 이상인 최댓값으로 가장 과거의 하한을 만들고, 인덱스로 1차 후보만 줄인다. 실제 노출 여부는 기존 `BETWEEN`이 그대로 판단한다.
...
`MAX(extra_days) >= 현재행.extra_days`라면 추가 하한은 현재 행의 실제 하한보다 같거나 과거다. 따라서 정상 범위의 값에서는 기존 조건을 만족하는 행을 새 조건이 제거하지 않는다. 추가 조건은 기존 결과의 상위집합을 읽는 인덱스용 가드다.
...
데이터의 `MAX(extra_days)`를 쓰면 이미 저장된 값까지 포함해 경계가 자동으로 따라간다. 정확성 실패보다 성능 저하를 관측하고 대응하는 편이 낫다는 판단이었다. 다만 이 선택이 공짜는 아니며, 3편에서 그 전제를 다시 검토한다.

## 비상관 서브쿼리는 한 번 계산돼 경계가 됐다
...
`DEPENDENT SUBQUERY`나 `UNCACHEABLE SUBQUERY`였다면 이야기가 달라진다. “서브쿼리는 느리다”가 아니라 **바깥 행에 의존하는지, 한 번 계산해 상수로 접을 수 있는지**를 봐야 한다.

## 성능과 결과를 따로 검증했다

공개용으로 수치를 범위화하면 결과는 다음과 같았다.
...
성능만 빨라진다고 끝내지 않았다. 같은 스냅샷에서 기존 조건과 새 조건의 결과 ID 집합을 비교했고, API 응답도 여러 필터와 페이징 조합으로 대조했다. 응답 동일성 검증은 2편에서 따로 다룬다.

이 시점의 결론은 “`MAX` 서브쿼리를 쓰자”가 아니었다.

- 원래 조건이 non-sargable한 이유를 먼저 찾는다.
- 원래 조건을 바꿀 수 없다면 더 느슨한 sargable 조건으로 후보를 줄일 수 있다.
- 추가 조건이 정말 상위집합인지 수학과 실제 데이터 양쪽에서 확인한다.
- 서브쿼리가 상수화되는지, 그 최댓값 조회가 어떤 인덱스에 기대는지 실행 계획으로 확인한다.

그리고 마지막 항목이 중요하다. 이 해법은 `MAX(extra_days)`가 정상 범위라는 전제와 `idx_content_item_extra_days`라는 숨은 의존을 새로 만들었다. 첫 성능 개선 뒤에 적대적 리뷰를 한 이유다.
