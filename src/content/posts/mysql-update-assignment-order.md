...
description: "두 컬럼을 함께 갱신할 때 기존 값을 읽는 IF가 SET 순서에 따라 달라지는 이유와 안전하게 다루는 기준을 정리한다."
kind: "tech"
...
MySQL에서는 그렇게 되지 않는다. 단일 테이블 UPDATE의 할당은 일반적으로 왼쪽에서 오른쪽으로 평가되고, 뒤에 있는 식은 앞선 할당으로 이미 바뀐 값을 볼 수 있다.
...
구현은 최초 승인 시각을 먼저 계산하는 순서로 정리했다. 다만 이제 이 순서는 단순한 포맷이 아니라 코드의 의미가 되었다. 누군가 컬럼을 보기 좋게 정렬하거나 공통 UPDATE를 합치면서 순서를 바꾸면 문법 오류 없이 동작만 달라진다.
...
| `NULL` | 기존 값 | 비정상 상태로 보고 별도 방어 여부 결정 |
...
여기서 사용한 순서 의존성은 MySQL의 단일 테이블 UPDATE에 대한 이야기다. 다중 테이블 UPDATE에서는 할당 순서가 보장되지 않는다. JOIN이 붙은 UPDATE로 바뀐다면 같은 방식에 기대면 안 된다.

MariaDB도 기본 단일 테이블 UPDATE는 왼쪽에서 오른쪽으로 할당하지만, `SIMULTANEOUS_ASSIGNMENT` SQL 모드에서는 모든 할당을 동시에 평가한다. PostgreSQL은 UPDATE 오른쪽 식에서 변경 전 행의 값을 사용한다고 명시한다. 같은 모양의 SQL이라도 서버와 설정이 바뀌면 결과가 달라질 수 있다.

그래서 이 패턴을 쓸 때는 범위를 좁혀 기록한다.
...
컬럼 두 개를 함께 갱신하는 짧은 SQL에서도 위에서 아래로 데이터가 어떻게 변하는지 한 줄씩 따라가야 한다. MySQL의 UPDATE에서 `SET` 순서는 정렬 취향이 아니라 동작의 일부다.
