---
title: "Claude Code에서 Java LSP: jdtls를 설치할까, IntelliJ /ide를 쓸까"
slug: "jdtls-intellij-lsp"
description: "Claude Code의 jdtls 플러그인과 IntelliJ /ide 연결이 제공하는 기능과 자원 비용을 비교해 선택 기준을 정리한다."
kind: "note"
...
jdtls는 설치 파일로 끝나지 않는다. Java 프로젝트를 메모리에 올리고 계속 인덱싱하는 별도 프로세스다.

당시 런처 스크립트를 확인했을 때 JVM 옵션은 `-Xms1G`였다.
...
처음에는 이를 최대 1GB로 잘못 읽었다. `-Xmx1G`가 아니라 `-Xms1G`이므로 1GB는 상한이 아니라 초기 힙이었다. 플러그인의 당시 `startupTimeout`도 120초로 설정돼 있었다. IntelliJ가 이미 같은 프로젝트를 인덱싱한 상태라면 분석 엔진을 하나 더 띄우는 셈이다.
...
내 첫 결론은 "IntelliJ가 있으니 jdtls는 낭비"였다. 확인 뒤에는 이렇게 바뀌었다.

- 진단만 필요하면 이미 실행 중인 IntelliJ와 `/ide`를 쓴다.
- 정밀한 코드 탐색이 필요하면 jdtls를 띄운다.
- IDE가 없는 환경에서는 jdtls가 언어 분석을 맡는다.

겹치는 기능은 진단이지만, 전체 기능은 같지 않았다. 도구를 고르는 기준은 중복 여부가 아니라 내가 지금 필요한 분석의 깊이였다.
