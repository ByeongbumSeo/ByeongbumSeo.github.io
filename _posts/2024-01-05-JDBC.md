...
**추가 개발 상식**
> JDBC 실행 과정 중 사용되는 Connection 객체를 미리 만들어 놓고 꺼내어 쓰는 기법이 **Connection Pool** (ex. HikariCP) 이다. 이러한 기법을 사용하면 Connection 생성 및 종료에 따른 오버헤드를 최소화하여 성능을 향상시킬 수 있다.  
> [HikariCP(Connection Pool) 포스팅 보러가기](https://byeongbumseo.github.io/posts/HikariCP/)

## Statement vs PreparedStatement
---
실무 환경에서는 Statement는 쓰지 않고 **PreparedStatement만 사용**하는데 그 이유는 뭘까?

우선 두 객체의 동작 방식에 대해서 알아본 뒤, 차이점을 비교해보자.

### Statement, PreparedStatement 동작 방식
...
---
JdbcTemplate은 JDBC Core Pakage의 중앙 클래스로, JDBC의 사용을 단순화하고 일반적인 오류를 방지하는 데 도움을 준다.  
커넥션 획득, statement 준비 등 개발자가 JDBC를 직접 사용할 때의 반복 작업을 대신 처리해준다.

즉, JdbcTemplate은 개발자가 JDBC 기술을 쉽게 사용할 수 있도록 도와주는 클래스다. spring-jdbc 라이브러리에 포함되어 있다.
