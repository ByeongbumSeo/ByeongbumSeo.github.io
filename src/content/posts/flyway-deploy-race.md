---
title: "머지 순서가 어긋나 운영이 멈춘 날"
slug: "flyway-deploy-race"
description: "각자 유효했던 Flyway 마이그레이션과 동시 배포가 만나 기동 실패와 중복 데이터를 만든 사고의 진단·수습 과정을 기록한다."
kind: "tech"
publishedAt: "2026-07-11"
draft: false
deprecated: false
outdated: false
tags: ["flyway", "database", "github-actions", "incident"]
series:
...
평범한 기능 두 개가 짧은 간격으로 머지된 밤, 운영 서버가 새 컨테이너를 띄우지 못했다. 낮은 버전의 Flyway 마이그레이션이 이미 더 높은 버전이 적용된 데이터베이스에 뒤늦게 들어온 게 시작이었다.

그 문제를 고치던 중에는 더 곤란한 일이 생겼다. 두 사람이 같은 백필 파일을 서로 다른 새 버전으로 각각 고쳤고, 두 파일이 모두 적용되면서 데이터가 두 번 들어갔다.

각 PR만 보면 유효했다. Flyway도 기록된 버전대로 정확히 동작했다. 빠져 있던 것은 뒤늦게 머지되는 마이그레이션의 버전을 검사하는 게이트와 애플리케이션 배포가 서로 겹치지 않게 하는 장치였다.
...
공개 글에서는 서비스 이름과 실제 버전, 배포 시각, 데이터 규모를 예시로 바꿨다. 사건의 순서와 판단 과정은 그대로다.
...
문제가 된 백필은 공개용 예시로 다음 두 작업을 함께 했다.
...
두 번 실행된 것은 한 배포가 아니라 **내용이 같은 서로 다른 versioned migration 두 개**였다. 데이터가 N배가 됐을 때 재시도 횟수만 볼 게 아니라, 실제 데이터를 쓰는 계층의 실행 이력을 먼저 확인해야 했다.
...
사람에게 “긴급 수정 전에 열린 PR을 확인하자”는 규칙도 필요하다. 하지만 장애 중 두 사람이 같은 해결책을 동시에 떠올리는 건 자연스럽다. 근본 대응은 사람의 주의력보다 마이그레이션 버전 검사를 머지 전에 수행하고, 애플리케이션 배포를 직렬화하며, 데이터베이스 제약으로 마지막 방어선을 세우는 것이었다.
...
이번 사고에서 Flyway는 약속한 대로 동작했다. 우리가 놓친 건 PR 하나의 유효성만으로 공유 DB의 버전 순서까지 보장되지는 않고, 내용이 같아도 버전이 다르면 별개의 마이그레이션으로 실행된다는 점이었다.
