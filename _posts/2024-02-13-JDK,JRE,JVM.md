...
그래서 오픈소스인 Java 소스코드를 빌드하여 사용하는 것에는 거의 제약이 없지만 Oracle에서 제공하는 설치버전의 JDK에는 NFTC(No-Fee Terms and Conditions) 라이센스가 적용되어 회사에서는 사용할 수 없다. (따로 구매해야함.)  
...
즉, Java로 짠 프로그램을 실행시키려면 반드시 필요하다. 그 이유는 JRE에 JVM에 포함되어 있기 때문이다.

그 외에도 대표적으로 표준 라이브러리를 포함하고 있다.
이 표준 라이브러리는 산술기능, 출력기능, 통신기능 등 널리 사용되는 다양한 기초, 필수 기능들을 직접 구현하지 않아도 되도록 미리 작성된 코드들을 말한다.

System.out.println, 

과거에는 Java를 실행만 할 컴퓨터(ex. 서버)에는 JRE만 사용해서 사용했다. 하지만 최근 버전에서는 굳이 JRE를 다운받는 것이 아닌, 이를 포함하는 JDK 전체를 다운받도록 하고 있다.
...
.java (프로그래머에 의해 코딩되어진 소스코드) -> 컴파일(javac.exe 컴파일러에 의해 변환) -> .class -> 클래스 로더에 의해 로딩 -> JVM에서 실행

.java => 사람이 코딩하기 위한 언어로된 파일
.class => 0 또는 1로 해석된 컴퓨터가 알아들을 수 있는 파일
...
전체 코드를 미리 읽지 않으므로 개발 시 실행하기 간편하지만 오류에 더 취약하다. 또한 실행속도도 상대적으로 느리다.
Javascript 등이 해당
...
자바를 실행'할' 컴퓨터 및 기기에 이 JVM을 설치해서 OS에 맞는 바이너리 코드로 변환이 가능하기 때문에, 기존에 '프로그램을 실행하는 OS에 맞춰 컴파일하던 것'에 대해 전혀 신경쓰지 않아도 된다.

전체 과정을 순서대로 작성하면 아래와 같다.

1. 작성된 코드를 Java 바이트 코드로 변환해둔다.
2. 실행하려는 쪽에서 자신에게 맞는(OS) JVM을 설치한다.
3. JVM이 바이트코드를 해당 OS에 맞는 실행 언어(바이너리 코드)로 변환해준다.
4. 이상없이 실행 가능.
